...
export default {
  async asyncData({ $axios, params }) {
    ...
    return {
      ...
      lendItemList: responseLendItemList.data.list, //投资记录列表
      lendReturnList: responseLendReturnList.data.list, // 还款记录列表
    }
  },
  ...
  methods: {
    ...
    commitInvest() {
      ...
      if (userInfoObj.userType != 1) {
        this.$message.error('借款类型用户无法进行投资')
        return
      }

      // 判断此次投标是否会导致该标的超卖，判断标准：已投资金额 + 本次投资金额 > 标的总金额
      if (
        this.lend.investAmount + Number(this.invest.investAmount) >
        this.lend.amount
      ) {
        this.$message.error('已经超出标的总金额,请修改本次投资金额')
        return
      }

      // 判断此次投标金额是否为 100 的整数倍
      if (
