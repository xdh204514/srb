...
export default {
  ...
  methods: {
    ...
    getBorrowAmount() {
      this.$axios
        ...
        .then((resopnse) => {
          this.borrowAmount = resopnse.data.borrowAmount
        })
    },
    ...
    initSelection() {
      //还款方式列表
      this.$axios
        .$get('/api/core/dict/getDictByDictCode/returnMethod')
        .then((response) => {
          this.returnMethodList = response.data.records
        })

      //资金用途列表
      this.$axios
        .$get('/api/core/dict/getDictByDictCode/moneyUse')
        .then((response) => {
          this.moneyUseList = response.data.records
        })
    },
  ...
  watch: {
    // borrowInfo.amount 有特殊字符，所以需要使用 ''，正常直接监听即可
    'borrowInfo.amount'(value) {
      if (value > this.borrowAmount) {
        let _this = this
        this.$message({
          message: '已经超过您的最高借款额度!',
          type: 'warning',
          duration: 1500,
          onClose: () => {
