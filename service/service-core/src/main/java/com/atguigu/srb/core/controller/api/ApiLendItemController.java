...
public class ApiLendItemController {
    ...
    public String notify(HttpServletRequest request) {
        ...
        log.info("会员投资异步回调：" + JSON.toJSONString(paramMap));
        ...
        if(RequestHelper.isSignEquals(paramMap)) {
            ...
            if("0001".equals(paramMap.get("resultCode"))) {
                ...
                log.info("会员投资异步回调失败：" + JSON.toJSONString(paramMap));
                return "fail";
            }
        } else {
            log.info("会员投资异步回调签名错误：" + JSON.toJSONString(paramMap));
            return "fail";
        }
    }
